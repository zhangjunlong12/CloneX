...
class MacOSAutomationService {
  static const _channel = MethodChannel('com.clonex/macos_automation');

  bool _isRecording = false;
  bool get isRecording => _isRecording;

  final List<MacOperation> _recordedOperations = [];
  List<MacOperation> get recordedOperations => List.unmodifiable(_recordedOperations);
  ...
  // 事件回调（用于实时显示录制的事件）
  void Function(MacOperation operation)? onOperationRecorded;
  ...
  /// 获取录制的事件
  Future<List<MacOperation>> getRecordedEvents() async {
    try {
      final result = await _channel.invokeMethod<List<dynamic>>('getRecordedEvents');
      if (result != null) {
        final operations = result
            .map((e) => MacOperation.fromJson(Map<String, dynamic>.from(e as Map)))
            .toList();
        _recordedOperations.clear();
  ...
  /// 回放操作序列
  Future<bool> play(List<MacOperation> operations) async {
